...
class ExampleRepositoryTest(
    ...
    Given("Example 엔티티가 여러개 주어지면") {
        ...
        When("saveIsAuth를 사용해서 JPQL로 DB의 값을 변경할 때") {
            ...
            val newExample1: Example? = exampleRepository.findById(example1.id!!).orElse(null)
            val newExample2: Example? = exampleRepository.findById(example2.id!!).orElse(null)
    ...
    xGiven("Example의 ID값이 주어지면") {
        val id: Long = 1L

        When("findByIdForLock를 사용해서 비관적 락(DB Lock)을 잡을 때") {
            val example: Example? = exampleRepository.findByIdForLock(id).orElse(null)
    ...
    Given("Example 엔티티 조회 후 주어지면") {
        val example: Example? = exampleRepository.findById(1L).orElse(null)
        ...
        When("엔티티의 값을 변경 후 JPQL을 호출할 때") {
            ...
            Then("(flush)update, select 순서대로 SQL이 나가고 JPQL로 조회된 List의 첫번째 값은 'newname' 이여야 한다.") {
                examples[0].name shouldBe "newname"
            }
        }
    }
})